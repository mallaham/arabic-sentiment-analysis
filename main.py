...
class SentimentAnalyzer(DataWorker):
    ...
    def predict(self, x_test):
        ...
        for model_name, fitted_model in self.fit_models.items():
            ...
            print(string.format(model_name, positive*100/pred.shape[0], negative*100/pred.shape[0]))
        return self.predictions

    def scoring(self, x_test, y_test):
        logging.info("Scoring classifiers...")
        for model_name, fitted_model in self.fit_models.items():
            pred = fitted_model.predict(x_test)
            scoring_metrics = self.calculate_metrics(pred, y_test)
            string = '\t{} | Accuracy. {:.2f}% F1. {:.2f}% Precision. {:.2f} Recall. {:.2f}'
            print(string.format(model_name, scoring_metrics[0] * 100, scoring_metrics[1] * 100,
...
if __name__ == "__main__":
    ...
    if score_data:
        test_data = arSA.process_data(arSA.test_data['txt'])
        test_target = arSA.test_data['sentiment']

    arSA.train_model(train_data, train_target)
    arSA.scoring(test_data, test_target)
